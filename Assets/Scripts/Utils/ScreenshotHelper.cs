...
public class ScreenshotHelper : MonoBehaviour
{
    ...
    {
        string filename = string.Format( folderPath + "/capture_{0}.png", DateTime.Now.ToString( "yyyy-MM-dd_HH-mm-ss-fff" ) );
        if (Directory.Exists( folderPath ) == false)
        {
            Directory.CreateDirectory( folderPath );
        }
        TakeTransparentScreenshot( targetCamera, Screen.width, Screen.height, filename );
    }
